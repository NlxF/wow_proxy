...
static _Bool xml_string_equals(struct xml_string* a, struct xml_string* b) {
	...
	size_t min_length = a->length > b->length ? b->length : a->length;

	//if (a->length != b->length) {
		//return false;
	//}
	
	// printf("compare string:%s(%d) and string:%s(%d)\n\n", a->buffer, a->length, b->buffer, b->length);
...
static struct xml_node* xml_parse_node(struct xml_parser* parser) {
	...
	//printf("one tag open is:%s, length:%d\n", tag_open->buffer, tag_open->length);
